...
void FMM2DTree::buildTree()
{
    ...
    // Now since we are dealing with a level-restricted tree, here we will traverse the tree
    // again to ensure that neighbors are not separated by more than one level:
    for(int i = 0; i < N_level; i++)
    {
        for(int j = 0; i < this->number_of_boxes[i]; i++)
        {
            for(int k = 0; k < 8; k++)
            {
